import argparse, os, sys, time, logging, socket, multiprocessing
...
def train_model(model, loader, optimizer, alpha, device):
    ...
    for clips, (pres_t, team_t) in loader:
        clips, pres_t, team_t = clips.to(device), pres_t.to(device), team_t.to(device)
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        logger.info(
            f"[Epoch {epoch}/{args.epochs}] Train: {train_loss:.4f}  Val: {val_loss:.4f}  (took {time.time() - t0:.1f}s)"
        )
